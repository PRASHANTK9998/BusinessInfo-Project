...
export class AddEditBusinessComponent implements OnInit, OnDestroy {
  ...
  buildForm(): void {
    this.addDataForm = this.formBuilder.group({
      ...
      // GSTIN: [this.getLoginUserData().GSTIN, [Validators.required]],
      email: [
  ...
  // Fetch all business data from the service
  // getAllBusinessData(): void {
  //   this.subscriptions$.add(
  //     this.apiService.getAllData().subscribe({
  //       next: (data: BusinessData[]) => {
  //         this.businessData = data;
  //       },
  //       error: (err) =>
  //         console.log('Error in getting data in the add edit compoenent' + err),
  //     })
  //   );
  // }
  ...
  saveDetails() {
    ...
    const newBusinessdata: Business = {
      ...
      // GSTIN: this.addDataForm.value.GSTIN,
      email: this.addDataForm.value.email,
      mobileNumber: this.addDataForm.value.mobileNumber,
    ...
    this.subscriptions$.add(this.apiService.updateUser(user).subscribe());
  }
